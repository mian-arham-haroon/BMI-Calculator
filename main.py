...
def BMI ():
    ...
    #you can change value ,becouse different contries has different bmi index

    if bmi<=18.5:
        label2.config(text="Underweight!")
        label3.config(text="You have lower wight then normal body")

    elif bmi>=18.5 and bmi<=25:
        label2.config(text="Normal!")
        label3.config(text="It indicate that you are healthy")

    elif bmi>=25 and bmi<=30:
        label2.config(text="Overweight!")
        label3.config(text="It indicate that a person is\n slightly overweight!\n A doctor may advise to lost some\n weight for health reasons!")
...
# Create and place an empty label for the image
secondimage = Label(root, bg="lightblue")
secondimage.place(x=70, y=310)  # Initial placement (adjust as needed)
def slider_changed(event):
    Height.set(get_current_value())
    
    size=int(float(get_current_value()))
    img=(Image.open("man.png"))
    resized_image=img.resize((50,10+size),Image.Resampling.LANCZOS)
